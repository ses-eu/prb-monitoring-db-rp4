...
if (country != rp_full) {
  data_calc <- data_raw %>%
    ...
    mutate(
      ...
      main_indicator_ans = if_else(
        is.na(main_indicator_ans),
        0,
        main_indicator_ans
      ),
      rate_per_100_000_with_ans_contribution = if_else(
        is.na(rate_per_100_000_with_ans_contribution),
        0,
        rate_per_100_000_with_ans_contribution
      ),

      variation_perc = if_else(
        lag(main_indicator, 1) == 0,
        10^6, # to identify the NAs, we'll change them back later
        main_indicator / lag(main_indicator, 1) - 1
      ) *
        100,
      rate_variation_perc = if_else(
        lag(rate_per_100_000, 1) == 0,
        10^6,
        rate_per_100_000 / lag(rate_per_100_000, 1) - 1
      ) *
        100,
      ans_variation_perc = if_else(
        lag(main_indicator_ans, 1) == 0,
        10^6,
        main_indicator_ans / lag(main_indicator_ans, 1) - 1
      ) *
        100,
      rate_ans_variation_perc = if_else(
        lag(rate_per_100_000_with_ans_contribution, 1) == 0,
        10^6,
        rate_per_100_000_with_ans_contribution /
          lag(rate_per_100_000_with_ans_contribution, 1) -
          1
      ) *
        100,
    ) %>%
  ...
  data_prep1 <- data_calc %>%
    ...
    mutate(
      ...
      mylabel = as.character(janitor::round_half_up(mymetric, 2))
    )
  ...
  data_prep2 <- data_calc %>%
    ...
    mutate(
      mylabel = if_else(
        ...
        paste0(
          if_else(mymetric >= 0, "+", ""),
          as.character(janitor::round_half_up(mymetric, 1)),
          "%"
        )
      ),
  ...
  data_prep1 <- data_raw %>%
    ...
    mutate(
      ...
      mylabel = as.character(janitor::round_half_up(value, 2))
    ) %>%
    select(
      ylabel,
      mylabel,
