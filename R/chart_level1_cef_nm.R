...
data_prep <- data_prep_wide %>%
  ...
  mutate(
    mymetric = if_else(type == "actual" & xlabel > year_report, NA, mymetric),
...
data_prep_costs <- data_prep_wide %>%
  ...
  mutate(
    myothermetric = if_else(
      type == "actual_costs" & xlabel > year_report,
      NA,
      janitor::round_half_up(myothermetric / 1000, 2)
    ),
...
p1 %>%
  add_line_trace2(
    .,
    filter(data_prep_costs, type == "Planned costs"),
    name = "Planned costs",
  ...
  add_line_trace2(
    .,
    filter(data_prep_costs, type == "Actual costs"),
    name = "Actual costs",
    textfontcolor = "rgba(0,0,0,0)",
  ...
  layout(
    yaxis2 = list(
      title = paste0("Total costs ('000 €", cef_ref_year, ")"),
      overlaying = "y",
      side = "right",
      showgrid = FALSE,
      showline = FALSE,
