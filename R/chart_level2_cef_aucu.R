  ...
  for (i in 1:nrow(data_prep)) {
    ...
    data_prep_filtered[[i]] <- data_prep %>%
      ...
      mutate(
        mydatalabel = case_when(
          type == 'total_adjustments_aucu' ~ if_else(
            is.na(metric) == TRUE,
            NA,
            paste0(
              if_else(metric >= 0, '+', ''),
        ...
        fake_series = case_when(
          type == 'total_adjustments_aucu' ~ min(
            c(aucu, new_duc),
            na.rm = TRUE
          ),
          .default = NA
    ...
    mychart[[i]] <- data_prep_filtered[[i]] %>%
      plot_ly(
        ...
        color = ~ factor(
          subtype,
          levels = c('new_duc', 'fake_series', 'total_adjustments_aucu', 'aucu')
        ),
        colors = c_barcolor,
        text = ~mydatalabel,
        textangle = -90,
