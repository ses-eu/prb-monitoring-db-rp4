...
fmt_pct <- function(x) {
  paste0(
    format(janitor::round_half_up(x * 100, 1), nsmall = 1, scientific = FALSE),
    "%"
  )
}
...
spec <- list(
  ...
  `% en route ATFM delay\n(traffic > forecast) (PI#4)` = list(
    src = "perc_delay_tp_above",
    fn = fmt_pct
  ),
  `Days with traffic > forecast\nAND en-route ATFM delay` = list(
    src = "days_where_tfc_guid_tfc",
    fn = fmt_int
  )
)
