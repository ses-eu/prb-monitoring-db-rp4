...
if (country == rp_full) {
  data_raw <- data_raw %>%
    ...
    summarise(
      duc_eur_ref = sum(total_cost_eur_ref, na.rm = TRUE) /
        sum(total_su, na.rm = TRUE),
      .groups = "drop"
    ) %>%
...
if (knitr::is_latex_output()) {
  if (
    (cztype == "terminal" & no_tcz > 1) | (cztype == "enroute" & no_ecz > 1)
  ) {
    c_level1_title <- paste0(
      " determined/actual unit\ncosts (DUC/AUC) - ",
      mycz_name
    )
  } else {
...
c_title_text <- if_else(
  doclevel == "level1",
  paste0(if_else(cztype == "terminal", "Terminal", "En route"), c_level1_title),
  "DUC/AUC"
)
