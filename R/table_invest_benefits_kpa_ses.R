...
data_prep <- data_benefit_ses %>% 
  filter(field %in% c("SAFETY benefits", "ENVIRONMENT benefits", "CAPACITY benefits", "COST-EFFICENCY benefits")) %>% 
  filter(status == "A") %>% 
...
table1 <- mygtable(data_prep, myfont) %>% 
  ...
  cols_label(
    field = html(paste0("Actual costs - expected benefits for KPA (M€<sub>",cef_ref_year,"</sub>)"))
  ) %>% 
  tab_style(
    style = list(
...
if (knitr::is_latex_output()) {
  table_inv_benefit_ses <- mylatex(table1)
