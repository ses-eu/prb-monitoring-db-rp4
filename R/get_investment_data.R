...
data_costs_sdo <- readxl::read_xlsx(
  here(data_folder, investments_data_file),
  sheet = "Output_Costs_MR_SDO",
  range = cell_limits(c(1, 1), c(NA, NA))
) %>%
  as_tibble() %>%
  clean_names() |>
...
data_funding_rt <- readxl::read_xlsx(
  here(data_folder, investments_data_file),
  sheet = "InputOutput_RT_Funding",
  range = cell_limits(c(1, 1), c(NA, NA))
) %>%
  as_tibble() %>%
  clean_names() |>
