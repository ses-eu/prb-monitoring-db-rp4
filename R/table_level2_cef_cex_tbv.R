...
data_prep_t2 <- data_raw %>%
  ...
  pivot_longer(
    cols = -c(entity_code, year, x4_7_total_su),
    names_to = 'type',
    values_to = 'mymetric'
  ) %>%
  mutate(
    xlabel = case_when(
      ...
      type ==
        "x3_3_cost_authority_qes" ~ "Competent authorities and qualified entities costs",
      ...
      type ==
        "x3_8_diff_det_cost_actual_cost" ~ "Total cost exempt from cost risk sharing"
    )
  ) %>%
  mutate(
