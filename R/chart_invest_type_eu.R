...
if (country == rp_full) {
  ...
  data_prep_ansp <- data_impact %>%
    ...
    mutate(
      type = "ANSP",
      `SES mandated` = if_else(
        .data[[denom]] == 0, 0,
        .data[[paste0("ses_rp", rp)]] / .data[[denom]]
      ) * 100,
      Partnership = if_else(
        .data[[denom]] == 0, 0,
        .data[[paste0("par_rp", rp)]] / .data[[denom]]
      ) * 100,
      `CP/MP investment` = if_else(
        .data[[denom]] == 0, 0,
        .data[[paste0("cpmp_rp", rp)]] / .data[[denom]]
      ) * 100
    ) %>%
    select(type, `SES mandated`, Partnership, `CP/MP investment`) %>%
    pivot_longer(-type, names_to = "xlabel", values_to = "mymetric")
...
myplot <- mybarchart2(data_prep, 
                      ...
                      yaxis_title = paste0("% of RP", rp," actual costs of new\ninvestments by type of investment"),
