...
data_prep <- data_funding %>%
  ...
  mutate(
    value = if_else(
      as.numeric(year) > year_report & year != rp_short,
      NA,
      value / 10^6
    )
  ) %>%
  ...
  mutate(
    type = case_when(
      tolower(type) ==
        "total self-declared funding" ~ "Total included in the funding declaration (reporting tables)",
      tolower(type) == "sdm data" ~ "SDM Payment",
      tolower(type) ==
        "difference" ~ "Difference between total funding declared vs received"
    )
  ) %>%
  arrange(year) %>%
