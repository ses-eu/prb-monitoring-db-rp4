...
data_prep_ses <- data_raw_cz %>%
  ...
  select(entity_name, year, status, su, costs, costs_eur_ref, duc)
...
data_prep_su <- data_prep |>
  pivot_wider(
    id_cols = -c(costs, costs_eur_ref, duc),
    values_from = "su",
    names_from = "status"
  ) |>
  mutate(
...
data_prep_costs <- data_prep |>
  pivot_wider(
    id_cols = -c(su, costs_eur_ref, duc),
    values_from = "costs",
    names_from = "status"
  ) |>
  mutate(
...
data_prep_costs_ref <- data_prep |>
  pivot_wider(
    id_cols = -c(su, costs, duc),
    values_from = "costs_eur_ref",
    names_from = "status"
  ) |>
  mutate(
    mymetric = (A / D - 1) * 100,
...
data_prep_duc <- data_prep |>
  pivot_wider(
    id_cols = -c(su, costs, costs_eur_ref),
    values_from = "duc",
    names_from = "status"
  ) |>
  mutate(
...
cef_plot <- function(df, xtitle) {
  plot_ly(
    ...
    x = ~mymetric,
    y = ~entity_name,
    text = ~ paste0(janitor::round_half_up(mymetric, 0), "%"),
    ...
    layout(
      ...
      xaxis = list(
        ...
        showgrid = FALSE,
        range = c(
          floor((min(df$mymetric) - 5) / 10) * 10 - 5,
          ceiling((max(df$mymetric) + 5) / 10) * 10 + 5
        )
      ),
      yaxis = list(
        title = '',
...
p1 <- cef_plot(data_prep_su, c_title1) |>
  layout(
    xaxis = list(
      range = c(
        floor((min(data_prep_su$mymetric) - 5) / 10) * 10 - 20,
