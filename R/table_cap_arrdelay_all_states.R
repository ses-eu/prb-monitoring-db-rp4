...
state_table <- state_list |>
  as_tibble() |>
  filter(value != "MUAC", value != rp_full, value != "Network Manager") |>
  select(State = value) |>
  arrange(State)
...
data_prep_actual <- data_raw_actual %>%
  filter(year == .env$year_report) %>%
  mutate(
    average_delay = if_else(
      is.na(average_delay_per_flight) == TRUE,
      0,
      janitor::round_half_up(average_delay_per_flight, 2)
    )
  ) |>
  select(
    State = state,
