...
```{r}
inv_conc1 <- get_prb_conclusions(prb_findings_file, "investments", "Table_Findings_investments")
```

`r inv_conc1`

## Value of the assets for New Investments included in the Performance Plan `r country` | `r if_else(params$country == params$rp_full, "all ANSPs", main_ansp)`
...
:::: {.chart-container}
```{r chart_invest_av_vs_eu2, eval=(params$country != params$rp_full)}
...
```{r table_invest_benefits_kpa, eval=!(params$country == params$rp_full)}
#| file: R/table_invest_benefits_kpa.R
#| out.width: "100%"
```

```{r table_invest_benefits, eval=!(params$country == params$rp_full)}
...
```{r table_invest_benefits_kpa_ses, eval=(params$country == params$rp_full)}
#| file: R/table_invest_benefits_kpa_ses.R
#| out.width: "100%"
```

```{r table_invest_impact_ses, eval=(params$country == params$rp_full)}
